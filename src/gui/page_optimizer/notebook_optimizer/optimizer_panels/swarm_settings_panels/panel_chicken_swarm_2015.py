...
class Chicken_Swarm_2015_Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.maxWeight = 0.9              # Constant float. maximum, starting weight
        self.minWeight = 0.4              # Constant float. minimum weight.
        self.learningFactor = 0.4         # Learning factor. Weight of rooster location for current particle sub-group
        self.metricArr = []
        ...
        stMaxWeight= wx.StaticText(boxTuning, label="Max Weight")
        self.fieldMaxWeight = wx.TextCtrl(boxTuning, value=str(self.maxWeight), size=(self.defaultBoxWidth,-1)) 
        stMinWeight = wx.StaticText(boxTuning, label="Min. Weight")
        self.fieldMinWeight = wx.TextCtrl(boxTuning, value=str(self.minWeight), size=(self.defaultBoxWidth,-1)) 
        stLearningFactor = wx.StaticText(boxTuning, label="Learning Factor")
        self.fieldLearningFactor = wx.TextCtrl(boxTuning, value=str(self.learningFactor), size=(self.defaultBoxWidth,-1)) 
        ...
        col3TuningSizer.AddSpacer(4)
        col3TuningSizer.Add(stMinWeight, 0, wx.ALL, border=5)
        col3TuningSizer.AddSpacer(4)
        col3TuningSizer.Add(stMaxWeight, 0, wx.ALL, border=5)
        col3TuningSizer.AddSpacer(4)
        col3TuningSizer.Add(stLearningFactor, 0, wx.ALL, border=5)
        ...
        col4TuningSizer.Add(self.fieldMinWeight, 0, wx.ALL, border=3)  
        col4TuningSizer.Add(self.fieldMaxWeight, 0, wx.ALL, border=3)  
        col4TuningSizer.Add(self.fieldLearningFactor, 0, wx.ALL, border=3)  
    ...
    def getInputFields(self):
        ...
        try:
            ...
            logger.error("ERROR in panel_chicken_swarm_2015 getting user input")
            logger.error(e)
            noError = False   
