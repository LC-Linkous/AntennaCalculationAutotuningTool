...
#   '.src/gui/page_optimizer/notebook_optimizer/optimizer_panels/bayesian_settings_panels/panel_knn_regression.py'
...
class KNNRegression_Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.KNN_n_neighbors=3
        self.KNN_weights='uniform'  #options: 'uniform', 'distance'
        ...
        stWeight = wx.StaticText(boxTuning, label="Weight Dist.")
        weightTypes = ['uniform', 'distance']
        self.weightDropDown = wx.ComboBox(boxTuning, choices=weightTypes, id=1,style=wx.CB_READONLY, size=(280, -1))
        self.weightDropDown.SetValue(weightTypes[0])
        stNeighbors = wx.StaticText(boxTuning, label="Num. Neighbors")
        self.fieldNeighbors = wx.TextCtrl(boxTuning, value=str(self.KNN_n_neighbors), size=(self.defaultBoxWidth,-1))     
        ...
        col3TuningSizer.Add(stWeight, 0, wx.ALL, border=5)
        col3TuningSizer.AddSpacer(4)
        col3TuningSizer.Add(stNeighbors, 0, wx.ALL, border=5)

        col4TuningSizer = wx.BoxSizer(wx.VERTICAL)
        col4TuningSizer.AddSpacer(10)
        col4TuningSizer.Add(self.weightDropDown, 0, wx.ALL, border=3)
        col4TuningSizer.Add(self.fieldNeighbors, 0, wx.ALL, border=3)   
    ...
    def getInputFields(self):
        ...
        try:
            ...
            self.KNN_n_neighbors = self.fieldNeighbors.GetValue()
            self.KNN_weights = self.weightDropDown.GetValue()
    ...
    def getOptimizerInputs(self, is_surrogate=False):
        ...
        if noError == True:   
            ...
            df['knn_n_neighbors'] = pd.Series(self.KNN_n_neighbors)
            df['knn_weights'] = pd.Series(self.KNN_weights)    
            df['sm_model_name'] = pd.Series([SM_KNN])
        return df, noError
