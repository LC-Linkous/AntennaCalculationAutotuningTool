...
#   '.src/gui/page_optimizer/notebook_optimizer/optimizer_panels/bayesian_settings_panels/panel_kriging_regression.py'
...
class Kriging_Panel(wx.Panel):
    def __init__(self, parent):
        ...
        #surrogate ID vars
        self.surrogateModel = 2

        #class variables
        self.toleranceVal = 1e-4            # Convergence Tolerance
        self.initialNumPts = 2              # Initial number of points (set minimum)
        ...
        stNoise = wx.StaticText(boxTuning, label="K Noise")
        self.fieldNoise = wx.TextCtrl(boxTuning, value=str(self.K_noise), size=(self.defaultBoxWidth,-1))
        stLengthScale = wx.StaticText(boxTuning, label="K Length Scale")
        self.fieldLengthScale = wx.TextCtrl(boxTuning, value=str(self.K_length_scale), size=(self.defaultBoxWidth,-1))
    ...
    def getOptimizerInputs(self, is_surrogate=False):
        ...
        if noError == True:   
            ...
            df['k_noise'] = pd.Series(self.K_noise)
            df['k_length_scale'] = pd.Series(self.K_length_scale)    
            df['sm_model_name'] = pd.Series([SM_KRIGING])
        return df, noError
