...
#   '.src/optimizers/PSO_BASIC/controller_PSO_BASIC.py'
#   Class interfacing with the PSO_BASIC optimizer
#
...
class CONTROLLER_PSO_BASIC():
    def __init__(self, parent): 
        ...
        self.sm_approx = None     # OBJECT. the surrogate approximator. The kernel
        self.sm_opt = None        # OBJECT. the internal optimizer used to solve the surrogate model.
...
######################################################
    ...
    def create_internal_optimizer(self, optimizerParams):
        ...
        if noError == False:
            msg = "ERROR in PSO_BASIC OPTIMIZATION CONTROLLER. Incorrect surrogate model configuration not recoverable by automated defaults."
            self.debug_message_printout(msg)
        ...
        opt_df = pd.DataFrame(opt_params)
        sm_optimizer = swarm # shouldn't need this because it's already been used to create the controller
