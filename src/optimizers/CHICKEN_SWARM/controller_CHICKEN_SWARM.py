...
#   '.src/optimizers/CHICKEN_SWARM/controller_CHICKEN_SWARM.py'
#   Class interfacing with the chicken swarm optimizer
#
...
import numpy as np
import pandas as pd
from optimizers.CHICKEN_SWARM.chicken_swarm_python.chicken_swarm import swarm
from optimizers.CHICKEN_SWARM.chicken_swarm_python.constr_default import constr_default


class CONTROLLER_CHICKEN_SWARM():
...
#######################################################
    ...
    def unpackOptimizerParameters(self, optimizerParams, func_F):
        ...
        ## CHICKEN_SWARM is the base optimizer. It can have an internal optimizer solving a surrogate model approximation.
...
######################################################
    ...
    def create_internal_optimizer(self, optimizerParams):
        ...
        if noError == False:
            msg = "ERROR in CHICKEN_SWARM OPTIMIZATION CONTROLLER. Incorrect surrogate model configuration not recoverable by automated defaults."
            self.debug_message_printout(msg)
