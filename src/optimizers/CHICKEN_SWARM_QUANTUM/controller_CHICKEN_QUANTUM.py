...
#   '.src/optimizers/CHICKEN_QUANTUM/controller_CHICKEN_QUANTUM.py'
#   Class interfacing with the chicken swasm quantum optimizer
#
    ...
    def unpackOptimizerParameters(self, optimizerParams, func_F):
        ...
        BETA = float(optimizerParams['beta'][0])                    # Float constant controlling influence 
                                                                    #    between the personal and global best positions
                                                                                                                                                      #              3 = absorbing,   4 = invisible
        QUANTUM_ROOSTERS = optimizerParams['quantum_roosters'][0]   # Boolean for is roosters also have random in their movement                                                                #              3 = absorbing,   4 = invisible
        ...
        ## CHICKEN_SWARM_QUANTUM is the base optimizer. It can have an internal optimizer solving a surrogate model approximation.
...
######################################################
    ...
    def create_internal_optimizer(self, optimizerParams):
        ...
        BETA = float(optimizerParams['sm_beta'][0])                    # Float constant controlling influence 
                                                                       #    between the personal and global best positions
                                                                                                                                                      #              3 = absorbing,   4 = invisible
        QUANTUM_ROOSTERS = optimizerParams['sm_quantum_roosters'][0]   # Boolean for is roosters also have random in their movement                                                                #              3 = absorbing,   4 = invisible
        ...
        if noError == False:
            msg = "ERROR in CHICKEN_QUANTUM OPTIMIZATION CONTROLLER. Incorrect surrogate model configuration not recoverable by automated defaults."
            self.debug_message_printout(msg)
