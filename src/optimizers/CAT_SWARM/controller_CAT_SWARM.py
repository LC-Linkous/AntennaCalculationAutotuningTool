...
#######################################################
    ...
    def unpackOptimizerParameters(self, optimizerParams, func_F):
        ...
        ## CAT_SWARM is the base optimizer. It can have an internal optimizer solving a surrogate model approximation.
...
######################################################
    ...
    def create_internal_optimizer(self, optimizerParams):
        ...
        WEIGHTS = [float(optimizerParams['sm_weights'][0])]                # Update vector weights
        VLIM = float(optimizerParams['sm_velocity_limit'][0])              # Initial velocity limit
        BOUNDARY = int(optimizerParams['sm_boundary'][0])                  # int boundary 1 = random,      2 = reflecting
                                                                           #              3 = absorbing,   4 = invisible

        # cat swarm specific
        MR = float(optimizerParams['sm_mixture_ratio'][0])                 # Mixture Ratio (MR). Small value for tracing population %.
        SMP = int(optimizerParams['sm_seeking_pool'][0])                   # Seeking memory pool. Num copies of cats made.
        SRD = float(optimizerParams['sm_seeking_range'][0])                # Seeking range of the selected dimension. 
        CDC = int(optimizerParams['sm_mutation_dim'][0])                   # Counts of dimension to change. mutation.
        SPC = bool(optimizerParams['sm_self_position'][0])                 # self-position consideration. boolean.          
        ...
        if noError == False:
            msg = "ERROR in CAT_SWARM OPTIMIZATION CONTROLLER. Incorrect surrogate model configuration not recoverable by automated defaults."
            self.debug_message_printout(msg)
