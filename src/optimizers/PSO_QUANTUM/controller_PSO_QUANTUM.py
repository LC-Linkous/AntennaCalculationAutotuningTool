...
#   '.src/optimizers/PSO_QUANTUM/controller_PSO_QUANTUM.py'
#   Class interfacing with the PSO_QUANTUM optimizer
#
...
import numpy as np
import pandas as pd
from optimizers.PSO_QUANTUM.pso_quantum.particle_swarm import swarm
from optimizers.PSO_QUANTUM.pso_quantum.constr_default import constr_default


class CONTROLLER_PSO_QUANTUM():
...
#######################################################
    ...
    def unpackOptimizerParameters(self, optimizerParams, func_F, internalOptimizerParams=None):
        ...
        ## PSO_QUANTUM is the base optimizer. It can have an internal optimizer solving a surrogate model approximation.
...
######################################################
    ...
    def create_internal_optimizer(self, optimizerParams):
        ...
        BETA = float(optimizerParams['sm_beta'][0])                       # Float constant controlling influence 
                                                                          #    between the personal and global best positions
        ...
        if noError == False:
            msg = "ERROR in PSO_QUANTUM OPTIMIZATION CONTROLLER. Incorrect surrogate model configuration not recoverable by automated defaults."
            self.debug_message_printout(msg)
