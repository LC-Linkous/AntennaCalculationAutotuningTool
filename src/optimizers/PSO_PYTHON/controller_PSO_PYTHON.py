...
#   '.src/gui/opimizers/PSO_PYTHON/controller_PSO_PYTHON.py'
#   Class interfacing with the PSO with time modulation optimizer
#
...
#######################################################
    ...
    def unpackOptimizerParameters(self, optimizerParams, func_F):
        ...
        ## PSO_PYTHON is the base optimizer. It can have an internal optimizer solving a surrogate model approximation.
        ...
        opt_params = {'BOUNDARY': [BOUNDARY],              # int boundary 1 = random,      2 = reflecting
                                                           #              3 = absorbing,   4 = invisible
                    'NO_OF_PARTICLES': [NO_OF_PARTICLES],  # Number of particles in swarm
...
######################################################
    ...
    def fit_and_create_surogate(self, opt_M, opt_F_Pb,surrogateOptimizer):
        ...
        func_f = self.model_predict  # NOTE: NO parenthesis. Passing the FUNCTION not the result
    ...
    def create_internal_optimizer(self, optimizerParams):
        ...
        WEIGHTS = [list(map(float, optimizerParams['sm_weights'][0]))]     # Update vector weights
        ...
        T_MOD = float(optimizerParams['sm_time_modulus'][0])               # Variable time-step extinction coefficient
        ...
        if noError == False:
            msg = "ERROR in PSO_PYTHON OPTIMIZATION CONTROLLER. Incorrect surrogate model configuration not recoverable by automated defaults."
            self.debug_message_printout(msg)
